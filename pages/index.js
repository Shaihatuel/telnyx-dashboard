...
const COLORS = ['#FF8C00', '#1E3A5F', '#4A90D9', '#F5A623', '#7B68EE', '#00C49F', '#FF6B6B', '#4ECDC4'];
...
export default function Dashboard() {
  ...
  useEffect(() => { if (selectedBrand !== 'all') fetchCampaigns(selectedBrand); else { setCampaigns([]); setSelectedCampaign('all'); } }, [selectedBrand]);
  ...
  const fetchCampaigns = async (brandId) => {
    try {
      ...
      const result = await res.json();
      if (result.data) { setCampaigns(result.data); if (result.data.length > 0) setSelectedCampaign(result.data[0].campaignId); }
    } catch (err) { console.error(err); }
  };
        ...
        <div className="bg-[#1E3A5F] rounded-xl shadow-lg border border-[#FF8C00]/20 p-5 mb-6">
          <div className="flex flex-wrap items-end gap-4">
            ...
            {campaigns.length > 0 && (<div><label className="block text-xs font-semibold text-gray-300 mb-2 uppercase">Campaign</label>
              <select value={selectedCampaign} onChange={(e) => setSelectedCampaign(e.target.value)} className="bg-[#0F1A2E] border border-[#FF8C00]/30 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-[#FF8C00] min-w-[200px]">
                <option value="all">All Campaigns</option>
                {campaigns.map((c) => (<option key={c.campaignId} value={c.campaignId}>{c.description || c.usecase || c.tcrCampaignId}</option>))}
              </select>
            </div>)}
        ...
        {data && !loading && (<>
          ...
          {activeTab === 'carriers' && (<div className="space-y-6">
            <div className="bg-[#1E3A5F] rounded-xl shadow-lg border border-[#FF8C00]/20 p-6"><h3 className="text-lg font-semibold text-white mb-4">Messages by Carrier</h3>
              <ResponsiveContainer width="100%" height={Math.max(400, data.carriers.length * 40)}><BarChart data={data.carriers} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#2D4A6F" /><XAxis type="number" tick={{ fill: '#9CA3AF' }} /><YAxis dataKey="carrier" type="category" width={150} tick={{ fontSize: 11, fill: '#9CA3AF' }} /><Tooltip formatter={(v, n) => n === 'Cost ($)' ? formatCurrency(v) : formatNumber(v)} contentStyle={{ backgroundColor: '#1E3A5F', border: '1px solid #FF8C00', borderRadius: '8px' }} /><Legend /><Bar dataKey="outbound" fill="#FF8C00" name="Outbound" radius={[0, 4, 4, 0]} /><Bar dataKey="inbound" fill="#4A90D9" name="Inbound" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer>
            </div>
